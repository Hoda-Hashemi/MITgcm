...
#!/usr/bin/env python3
"""
Generate initial conditions for vortexSphere_mitgcm_referenceCase.

EXPERIMENT1 : spits: eta_init.bin
Creates a compact cosine free-surface anomaly (meters) on the 1440x720
global 0.25-degree lat-lon grid, applied only over ocean points.
Expecting Fast Gravity Waves

Experiment2: 
eta_init.bin, u_init.bin, and v_init.bin initialize a geostrophically balanced
Gaussian free-surface patch.

The Gaussian η anomaly is defined in meters on the global 1440 x 720
0.25° lat-lon grid and applied only over ocean cells using
bathymetry-based land masking.

The velocity fields are initialized from the geostrophic/Kuo balance
fψ = gη, with u and v placed on the MITgcm C-grid velocity faces and
masked over inactive land faces.

This creates a balanced rotating eddy-like initial condition with reduced
gravity-wave adjustment compared with the cosine bump at rest.
