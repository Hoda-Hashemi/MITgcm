...
c     INTEGER actualIts
      ...
      Real*8    sigma
      Real*8    delta,  deltaTile(nSx,nSy)
      Real*8    sumRHS, sumRHStile(nSx,nSy)
      Real*8    sumVec(3), tiledVec(nSx,nSy,3)
      Real*8    rhsMax
      Real*8    rhsNorm
      Real*8    cg2d_min(1:sNx,1:sNy,nSx,nSy)
      Real*8    cg2d_v(1:sNx,1:sNy,nSx,nSy)
      Real*8    cg2d_q(1:sNx,1:sNy,nSx,nSy)
      Real*8    cg2d_y(1-1:sNx+1,1-1:sNy+1,nSx,nSy)
...
C-    Initialise local working arrays:
        DO j=1,sNy
         DO i=1,sNx
          cg2d_v(i,j,bi,bj) = 0.D0
          cg2d_q(i,j,bi,bj) = 0.D0
         ENDDO
        ENDDO
        DO j=0,sNy+1
         DO i=0,sNx+1
          cg2d_y(i,j,bi,bj) = 0.D0
