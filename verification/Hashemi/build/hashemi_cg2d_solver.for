...
C    !===  GRID SPACING & DOMAIN SIZE =====================================
      ...
      PARAMETER ( sNy =  360 )        ! grid points in y-dir per tile
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8 dLat, dLat2, lat, sinLat, cosLat, cosLat2
      Real*8 latMHalf, latPHalf, cosLatMHalf, cosLatPHalf
      Real*8 R2, coriolisTerm, coriolisTerm2
      Real*8 dC, dW, dE, dS, dN
      Real*8 coefWest, coefEast, coefSouth, coefNorth
...
C                     = 0 (not convecting) or 1 (convecting)
     ...
     &                hMixLayer, IVDConvCount
      ...
      Real*8 dPsiDy, dPsiDx, denY, denX
...
C--         BUGTRACKING: Wet-aware centered stencil test for coastal-noise diagnosis.
            IF ( maskW(i,j+1,1,bi,bj) .EQ. oneRL .AND.
     &           maskW(i,j-1,1,bi,bj) .EQ. oneRL ) THEN
...
C--        otherwise one-sided; if no wet support, set to zero.
          wetS0 = maskS(i  ,j,1,bi,bj) .EQ. oneRL
          wetSm = maskS(i-1,j,1,bi,bj) .EQ. oneRL
          wetSp = maskS(i+1,j,1,bi,bj) .EQ. oneRL
          ...
          uVel(i,j,1,bi,bj) = -dPsiDy * maskW(i,j,1,bi,bj)
          vVel(i,j,1,bi,bj) =  dPsiDx * maskS(i,j,1,bi,bj)
         ENDDO
        ENDDO
       ENDDO
      ENDDO
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8 r(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 z(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 p(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 q(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 resSq, rhsSq, rz, rzNew, pA
      Real*8 alphaCG, betaCG, tol, stopSq, eps
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8 xFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 yFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER myThid
      INTEGER bi, bj, i, j
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8 rFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 zFld(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      INTEGER myThid

      INTEGER bi, bj, i, j
      Real*8 zTmp(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 rowL(1:sNx), rowD(1:sNx), rowU(1:sNx)
      Real*8 rowR(1:sNx), rowX(1:sNx)
      Real*8 colL(1:sNy), colD(1:sNy), colU(1:sNy)
      Real*8 colR(1:sNy), colX(1:sNy)
      Real*8 eps, den
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      INTEGER bi, bj, i, j, it, actualIts
      Real*8 rhsInf, resInf, relRes, tol
      Real*8 rowL(1:sNx), rowD(1:sNx), rowU(1:sNx)
      Real*8 rowR(1:sNx), rowX(1:sNx)
      Real*8 colL(1:sNy), colD(1:sNy), colU(1:sNy)
      Real*8 colR(1:sNy), colX(1:sNy)
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
     ...
     &       +aC2d(i  ,j  ,bi,bj)*cg2d_x(i  ,j  ,bi,bj)
      ...
      CALL GLOBAL_MAX_R8 (  resInf,  myThid  )
      CALL GLOBAL_MAX_R8 (  rhsInf,  myThid  )
      RETURN
      END
...
C    !===  GRID SPACING & DOMAIN SIZE =====================================
     ...
     &            MAX_OLY = OLy )
      ...
      Real*8 aL(1:sNx), aD(1:sNx), aU(1:sNx)
      Real*8 rhs(1:sNx), sol(1:sNx)
      INTEGER i
      Real*8 cP(1:sNx), dP(1:sNx), den, eps
...
C    !===  GRID SPACING & DOMAIN SIZE =====================================
     ...
     &            MAX_OLY = OLy )
      ...
      Real*8 aL(1:sNy), aD(1:sNy), aU(1:sNy)
      Real*8 rhs(1:sNy), sol(1:sNy)
      INTEGER j
      Real*8 cP(1:sNy), dP(1:sNy), den, eps
...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8 tol
      Real*8 aC, aCw, aCs
      Real*8 aWloc, aEloc, aSloc, aNloc
      Real*8 diagAbs, dirX, dirY
      Real*8 denW, denS, tauX, tauY
      Real*8 adiTheta, adiShift, epsPre
