...
C     == Local variables ==
      INTEGER i, j
      Real*8 Zip, Zij, Zpj, Dim, Dij, Dmj, uD2, vD2
      Real*8 uD4(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 vD4(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C and should be less than 1/8, for example viscAh=0.01
     ...
     &                                ( Zip-Zij )*recip_DYG(i,j,bi,bj)
          vD4(i,j) = viscA4Z*recip_hFacS(i,j,k,bi,bj)*
     &               cosFacV(j,bi,bj)*( Zpj-Zij )*recip_DXG(i,j,bi,bj)
