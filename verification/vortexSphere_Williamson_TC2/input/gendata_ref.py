...
# Rotation angle
# Use ALPHA_DEG = 0.0 for the zonal case
# Change to other TC2 angles if needed
ALPHA_DEG = 0.0
...
def make_tc2_eta():
    ...
    mu = (
        -np.cos(lon_c) * np.cos(lat_c) * np.sin(ALPHA)
        + np.sin(lat_c) * np.cos(ALPHA)
    )
...
def make_tc2_u():
    ...
    u = U0 * (
        np.cos(lat_u) * np.cos(ALPHA)
        + np.cos(lon_u) * np.sin(lat_u) * np.sin(ALPHA)
    )
