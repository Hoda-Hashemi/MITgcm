...
# TC1 free-surface cosine-bell parameters
...
def write_flat_bathymetry(base_dir: Path):
    ...
    print(f"Wrote {BATHYMETRY_FILENAME}: bathymetry={BATHYMETRY} m")
...
def make_tc1_fields(alpha_rad: float = 0.0):
    ...
    # Cosine bell at free-surface cell centers.
    r = spherical_distance_rad(lon2d, lat2d, LON0_DEG, LAT0_DEG)

    eta = np.zeros((NY, NX), dtype=np.float64)
    inside = r < RADIUS_RAD
    eta[inside] = 0.5 * ETA0 * (1.0 + np.cos(np.pi * r[inside] / RADIUS_RAD))
    ...
    v[non_polar, :] = (psi_w[non_polar, :] - psi_e[non_polar, :]) / dx[non_polar, None]
...
def main():
    ...
    eta, u, v = make_tc1_fields(alpha_rad=ALPHA_RAD)
