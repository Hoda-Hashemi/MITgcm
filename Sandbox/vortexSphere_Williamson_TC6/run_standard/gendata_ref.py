...
BATHY_FILE = "bathymetry_flat_tc6.bin"
...
def make_velocity_fields(alpha_rad: float = ALPHA_RAD) -> tuple[object, object]:
    ...
    u = wf.tc6_u(lon_u, lat_u)
    v = wf.tc6_v(lon_v, lat_v)
    v[0, :] = 0.0
    v[-1, :] = 0.0
    return u, v
...
def main() -> None:
    ...
    print("Done.")
    print(f"H0          = {H0:.12f} m")
    print(f"wave number = {wf.TC6_WAVE_NUMBER}")
    print(f"K           = {wf.TC6_K:.12e} s^-1")
