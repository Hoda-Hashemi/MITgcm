...
def grid_corners():
    """
    Grid corners used by MITgcm's fCoriG read path.
    """
    lon_g = np.arange(NX, dtype=np.float64) * DLON_RAD
    lat_g = -0.5 * np.pi + np.arange(NY, dtype=np.float64) * DLAT_RAD
    return np.meshgrid(lon_g, lat_g, indexing="xy")

def rotated_mu(lon_rad: np.ndarray, lat_rad: np.ndarray, alpha_rad: float) -> np.ndarray:
    return (
...
def write_rotated_coriolis(base_dir: Path, alpha_rad: float = ALPHA_RAD) -> None:
    ...
    write_field(base_dir, FCORI_C_FILE, 2.0 * OMEGA * mu_c)
    write_field(base_dir, FCORI_G_FILE, 2.0 * OMEGA * mu_g)
    write_field(
        base_dir,
        FCORI_COS_FILE,
        2.0 * OMEGA * np.sqrt(np.maximum(0.0, 1.0 - mu_c * mu_c)),
    )
