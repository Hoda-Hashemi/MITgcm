#!/usr/bin/env python3
"""Preflight CFL report for Williamson TC1.

This is a read-only helper. It does not compile, submit jobs, or write model
inputs. It reuses the TC1 velocity-generation code and reports the advective
CFL implied by the current job alpha values and input/data deltaT.
...
cd /home/hmh85/scratch/MITgcm/Sandbox/vortexSphere_Williamson_TC1
python3 tools/check_cfl_tc1.py --all
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Report TC1 advective CFL before compiling or running MITgcm."
    )
    parser.add_argument(
        "--case",
        action="append",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="check all job_tc1_*.slurm cases. This is the default.",
    )
    parser.add_argument(
        "--warn-threshold",
        type=float,
        default=WARN_THRESHOLD,
        help=f"CFL value above which status becomes WARN; default {WARN_THRESHOLD}.",
    )
    parser.add_argument(
        "--fail-threshold",
        type=float,
        default=FAIL_THRESHOLD,
        help=f"CFL value above which status becomes FAIL; default {FAIL_THRESHOLD}.",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
...
def parse_repeat_grid(value: str, name: str) -> Tuple[int, float]:
    ...
    match = re.fullmatch(r"(\d+)\s*\*\s*([+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[EeDd][+-]?\d+)?)", value)
    if not match:
...
def parse_data_file(path: Path) -> GridConfig:
    ...
    missing = sorted({"deltaT", "rSphere", "delX", "delY"} - values.keys())
    if missing:
...
def parse_size_file(path: Path) -> Tuple[int, int]:
    ...
    missing = sorted({"sNx", "sNy", "nSx", "nSy", "nPx", "nPy"} - params.keys())
    if missing:
        raise ValueError(f"{path} is missing: {', '.join(missing)}")
...
def discover_jobs(jobs_dir: Path) -> List[JobCase]:
    jobs: List[JobCase] = []
    for path in sorted(jobs_dir.glob("job_tc1_*.slurm")):
        text = path.read_text()
        name = parse_export_value(text, "CASE_NAME")
        alpha = parse_export_value(text, "ALPHA_VALUE")
        if name is None or alpha is None:
            continue
        jobs.append(JobCase(name=name, alpha=safe_eval_number(alpha), path=path))
    if not jobs:
        raise ValueError(f"no TC1 job files found in {jobs_dir}")
    return jobs


def load_gendata_module(path: Path):
    spec = importlib.util.spec_from_file_location("tc1_gendata_ref", path)
...
def validate_consistency(grid: GridConfig, size_shape: Tuple[int, int], gendata_module) -> List[str]:
    ...
    if (grid.nx, grid.ny) != (size_nx, size_ny):
        warnings.append(
            f"input/data grid {grid.nx}x{grid.ny} does not match SIZE.h {size_nx}x{size_ny}"
        )
    ...
    if not math.isclose(grid.r_sphere, module_radius, rel_tol=0.0, abs_tol=1.0e-6):
        warnings.append(
            f"input/data rSphere={grid.r_sphere:g} differs from gendata_ref.py R_EARTH={module_radius:g}"
        )
    return warnings
...
def compute_report(
    ...
    _tracer, u, v = gendata_module.make_tc1_fields(alpha_rad=job.alpha)
    ...
    cfl_x = np.abs(u) * grid.delta_t / dx[:, None]
    cfl_y = np.abs(v) * grid.delta_t / dy
    ...
    if max_total > 0.0:
        suggested_delta_t = grid.delta_t * warn_threshold / max_total
    else:
...
def print_report(
    ...
    header = (
        "case   alpha(rad)  status   max_CFL_x   max_CFL_y   "
    ...
    for report in reports:
        ...
        print(
            f"{report.case.name:<5} "
            f"{report.case.alpha:11.6g}  "
            f"{report.status:<6} "
            f"{report.max_x:11.6g} "
            f"{report.max_y:11.6g} "
            f"{report.max_total:11.6g} "
            f"{report.location_lon:10.4f} "
            f"{report.location_lat:10.4f} "
            f"{dt_text:>15}"
        )
...
def main() -> int:
    ...
    jobs = discover_jobs(case_dir / "jobs" / "large")
