...
Audit date: 2026-06-17, Asia/Beirut.
...
| TC4 | Output completed; formal diagnostics next | `run_u0_20` completed through day 5 on normal/onode08; fields are finite and the translating low follows the analytic path | Output checked, validation pending |
...
TC4 is no longer waiting on output, but it should not be called finally validated until formal exact-error and forcing-budget diagnostics are archived. TC5 should not be submitted as a final science run until the non-finite output cause is fixed.
...
| TC4 | Forced translating low should keep its shape while moving eastward for 5 days | Analytic TC4 height and velocity fields generated by `input/gendata_ref.py`; forcing hook is active | `code/tc4_forcing.F`, `code/external_forcing_surf.F`, completed `run_u0_20` output | Output completed; first analytic path/error check looks right | Archive formal exact-error and forcing-budget diagnostics before final validation |
